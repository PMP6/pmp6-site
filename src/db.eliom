...
module Type = struct
  ...
  type _ field += Time : Time.t field
  ...
  let get_coding : type a. _ -> a field -> a Field.coding =
   fun driver_info field ->
    match field with
    | Time -> time_coding
    | field -> Option.value_exn (Field.coding driver_info field)

  let () = Caqti_type.Field.define_coding Time { get_coding }
  let time = field Time
  let ( & ) = tup2
...
include Monad.Make (struct
  ...
  let return x (module C : C) = Lwt_result.return x
...
let check_foreign_keys_pragma (module C : C) =
  C.find (Caqti_request.find ~oneshot:true Type.unit Type.bool "PRAGMA foreign_keys") ()
...
let run_keep_errors request = Caqti_lwt.Pool.use request pool
...
let collect ~in_:(input_type, input) ~out:(output_type, make_item) query (module C : C) =
  let%map.R items =
    C.fold
...
let collect_all ~out query = collect ~in_:unit ~out query

let find ~in_:(input_type, input) ~out:(output_type, make_item) query (module C : C) =
  let%map.R result = C.find (Caqti_request.find input_type output_type query) input in
  make_item result

let find_opt ~in_:(input_type, input) ~out:(output_type, make_item) query (module C : C) =
  let%map.R result =
    C.find_opt (Caqti_request.find_opt input_type output_type query) input
  in
  Option.map ~f:make_item result

let exec ~in_:(input_type, input) query (module C : C) =
  C.exec (Caqti_request.exec input_type query) input

let exec_with_affected_count ~in_:(input_type, input) query (module C : C) =
  C.exec_with_affected_count (Caqti_request.exec input_type query) input
