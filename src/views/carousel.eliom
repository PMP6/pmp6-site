...
let images () =
  let img_uri filename = Skeleton.Static.img_uri [ "carousel"; filename ] in
  List.map
...
let slide ~is_active ~src ~caption =
  H.li
    ~a:[ H.a_class @@ Utils.cons_is_active is_active [ "orbit-slide" ] ]
    [
      H.figure
...
let make_elt images =
  let open H in
  div_classes
    [ "orbit"; "carrousel-photos-plongeur" ]
    ~a:
      [
        a_user_data "orbit" "";
        a_user_data
          "options"
          "animInFromLeft:fade-in; animInFromRight:fade-in; animOutToLeft:fade-out; \
