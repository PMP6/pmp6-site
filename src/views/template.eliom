...
let favicon_link ~size =
  let favicon_filename = Printf.sprintf "favicon-%dx%d.png" size size in
  let favicon_uri = Skeleton.Static.img_uri [ "favicons"; favicon_filename ] in
  H.(
    link
      ~rel:[ `Icon ]
...
let menu_title_entry () = H.(li ~a:[ a_class [ "show-for-large" ] ] [ menu_title () ])

let menu_toggle () =
  let open H in
  div_class
    "title-bar"
    ~a:[ a_user_data "responsive-toggle" "top-menu"; a_user_data "hide-for" "large" ]
    [
      button
        ~a:
          [
...
let top_left_menu items =
  ...
  let rec entry (content, hierarchical_site_item) =
    ...
    | Eliom_tools.Disabled -> None
    | Eliom_tools.(Site_tree (main_page, items)) ->
        let li_class, submenu =
          match items with
          | [] -> ([], [])
          | _ :: _ ->
        ...
        in
        let a_elt =
          match main_page with
          | Not_clickable -> Raw.a [ content ]
          | Default_page srv | Main_page srv ->
              let (Srv service) = srv in
              a ~service [ content ] ()
        in
        Some (li ~a:li_class (a_elt :: submenu))
  in
  ul
    ~a:
      [
        a_class [ "menu"; "vertical"; "large-horizontal" ];
        a_user_data "responsive-menu" "accordion large-dropdown";
...
let search_form () =
  let open H in
  H.Form.get_form ~a:[ a_class [ "search-form" ] ] ~service:Google.Search.service
  @@ fun (as_sitesearch, q) ->
  [
...
let top_right_menu user =
  let open H in
  let connection = li [ Auth.View.Widget.connection_icon () ] in
  let user_menu = Auth.View.Widget.user_menu () in
  let admin = li [ Admin.View.Widget.interface_icon () ] in
  let search = li ~a:[ a_class [ "menu-text"; "search-form" ] ] [ search_form () ] in
  ul
    ~a:
      [
        a_class [ "menu"; "vertical"; "large-horizontal" ];
...
let header user =
  ...
  header
    [
      menu_toggle ();
      div
        ~a:[ a_class [ "top-bar"; "stacked-for-medium" ]; a_id "top-menu" ]
        [
...
let return_page { Content.title; in_head; in_body } =
  let (_ : unit Eliom_client_value.t) = Caml.([%client Foundation.init ()]) in
