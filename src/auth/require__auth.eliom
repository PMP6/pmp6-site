...
module Syntax = struct
  let ( let$ ) require handler = require handler
end
...
module Fallback = struct
  ...
  let forbidden _gp _pp = Content.redirection Service.forbidden
  let already_connected _gp _pp = View.Page.already_connected ()
end
...
module Permission : sig
  ...
  let make f = f
  let check perm user = perm user || Model.User.is_superuser user

  let session_check perm =
    match%map.Lwt Session.get_user () with None -> false | Some user -> check perm user
...
let has_permission perm handler =
  Eliom_tools.wrap_handler Session.get_user Fallback.login_required (fun user gp pp ->
      if Permission.check perm user then handler user gp pp else Fallback.forbidden gp pp)
...
let unauthenticated handler =
  Eliom_tools.wrap_handler Session.get_unauthenticated Fallback.already_connected handler
